...
export const getEtsiRoleLabel = (role: string): { label: string; color: string } => {
    if (role.includes('102232_IRI') || role.includes('IRI_Header'))  return { label: 'IRI Header',     color: '#ff3366' };
    if (role.includes('102232_CC') || role.includes('CC_Payload'))   return { label: 'CC Payload',     color: '#ff6600' };
    if (role.includes('102232'))                                      return { label: 'ETSI 102 232',   color: '#ff3366' };
    if (role.includes('TS101671_HI2'))                                return { label: 'HI2 IRI',        color: '#ff3366' };
    if (role.includes('TS101671_HI3'))                                return { label: 'HI3 CC',         color: '#ff6600' };
    if (role.includes('3GPP') || role.includes('33108'))              return { label: '3GPP LI',        color: '#a855f7' };
    if (role.includes('LIID') || role.includes('TargetIdentifier'))   return { label: 'LIID',           color: '#f59e0b' };
    if (role.includes('Timestamp'))                                   return { label: 'LI Timestamp',   color: '#34d399' };
    if (role.includes('CommIdentifier'))                              return { label: 'Comm ID',        color: '#60a5fa' };
    if (role.includes('SeqNum'))                                      return { label: 'Seq Num',        color: '#94a3b8' };
    if (role.includes('OID_Discriminator'))                           return { label: 'OID',            color: '#fbbf24' };
    if (role.includes('CC_'))                                         return { label: 'CC Field',       color: '#fb923c' };
