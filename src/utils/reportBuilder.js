...
function analyzeCryptoSignature(autocorr) {
    if (autocorr.length < 64)
        return { findings: [], baselineNoise: 0 };
...
function classifyEntropy(avgEntropy) {
    if (avgEntropy > 7.5) {
        return {
...
function extractArtifacts(nodes, fileData) {
    const artifacts = [];
...
// ─── Threat Score ─────────────────────────────────────────────────────────────
function computeThreatScore(avgEntropy, findings, hasStructure) {
    let score = 0;
...
function threatScoreEmoji(score) {
    if (score >= 75)
        return '🔴 CRITICAL';
    if (score >= 50)
        return '🟠 HIGH';
...
// ─── Main Report Builder ──────────────────────────────────────────────────────
export function generateIntelReport(params, fileData) {
    ...
    const structureTree = (parsed_structures ?? []).slice(0, 20).map(n => `| 0x${n.tag.toString(16).toUpperCase().padStart(4, '0')} | ${n.name} | ${n.is_container ? '📦 Container' : '📄 Primitive'} | ${formatBytes(n.value_length)} |`).join('\n');
 ...
 */
export function downloadReport(markdown, fileName) {
